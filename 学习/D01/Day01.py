...
'滤波器'
#滤波器是处理信号的工具，可以去除噪声、增强特定频率成分等。
#滤波器可以是低通、高通、带通或带阻等类型。
#低通滤波器：只允许低频信号通过，去除高频噪声。
#高通滤波器：只允许高频信号通过，去除低频成分。
#带通滤波器：只允许特定频率范围内的信号通过。
#带阻滤波器：阻止特定频率范围内的信号通过。

'滤波器分类'
#FIR（有限脉冲响应）和IIR（无限脉冲响应）滤波器是两种常见的数字滤波器类型。
#FIR滤波器：响应有限，计算简单，稳定性好，但需要更多的系数。
#IIR滤波器：响应无限，计算复杂，效率高，但可能不稳定。

'数学公式'
#FIR滤波器的数学公式：
# y[n] = Σ (k=0 to M) h[k] * x[n-k]
#其中，y[n] 是输出信号，x[n] 是输入信号，h[k] 是滤波器系数，M 是滤波器阶数。
#IIR滤波器的数学公式：
# y[n] = Σ (k=0 to M) b[k] * x[n-k] - Σ (j=1 to N) a[j] * y[n-j]
#其中，y[n] 是输出信号，x[n] 是输入信号，b[k] 是前向系数，a[j] 是反馈系数，M 和 N 分别是前向和反馈的阶数。
#对比：
#FIR滤波器：结构简单，绝对稳定，线性相位，计算量大，硬件实现方便，适合高精度场合。
#IIR滤波器：结构复杂，可能不稳定，非线性相位，计算量小，硬件实现复杂，适合实时处理和资源受限场合。
